# 本地 LLM + OpenClaw 兩機架構完整架設教程：一份讓你少走彎路的指南
...
> 適用對象：想在自己的 Linux 機器（PC）上跑本地 AI 推理引擎，再透過 Mac 上的 OpenClaw 統一管理 Telegram bot、網頁 UI、視覺辨識，而且完全不想依賴任何外部 API key（因為我們都知道那些帳單長什麼樣子）。
>
...
## 模型選擇指南（拜託先讀這段，不然後面都白做）

> **這是本教程最重要的段落。** 選錯模型 = 浪費時間。我們替你測了 13 個模型，只有 2 個能在 OpenClaw 環境下穩定使用。其他 11 個的死法各有千秋，詳見下方。
...
OpenClaw 的完整 system prompt（含 tool schemas + workspace files）達 **12,000+ tokens**。這對模型的 tool calling 能力是嚴峻的考驗——嚴峻到大部分模型直接棄考：

- **小模型（3B-9B）** 的「注意力」被巨大的 system prompt 佔滿，分不清該呼叫哪個 tool
- **部分模型不支援 Ollama tools API**（如 gemma3、phi4），連資格都沒有
- **部分模型格式不對**（如 llama 系列），只會把 JSON 當文字吐出來，不是真正呼叫 tool

**我們花了最多時間才搞懂的事：AGENTS.md 的指令品質比模型大小更重要。** `qwen3-vl:8b-instruct` 一開始也失敗了（tool name 搞錯），但在我們認真寫好 AGENTS.md 之後就穩了。與其花錢升級顯卡，不如先把指令寫對。詳見下方「AGENTS.md 設定」段落。

### 完整模型實測結果（RTX 5070 Ti 16GB，OpenClaw 2026.3.13）

#### OpenClaw Tool Calling 實測

所有模型在相同環境下測試：12K+ token system prompt、搜尋（exec + searxng-search）、燈控（exec + tradfri）、複合操作。公平競爭，結果殘酷。
...
> **qwen3-vl 沒有中間尺寸：** 8B 之上直接跳到 30B-A3B（Q4_K_M 需 20GB），16GB VRAM 塞不下。8B 就是 16GB 顯卡的天花板了，接受現實吧。
...
> 8B 模型只用 6-8 GB VRAM，即使 32K ctx 速度也不掉。16GB 顯卡有大量餘裕，這種感覺真好。
...
> 14B 模型在 ctx <= 16384 時穩定 79 tok/s，但 32K ctx 直接暴跌到 19 tok/s（VRAM 接近滿載，模型開始掙扎）。OpenClaw 最低要求 ctx 16000，所以只能設 16384，速度比 qwen3-vl:8b 的 131 tok/s 慢 40%。再加上沒有 vision、tool calling 品質較差（喜歡碎碎念），真的不推薦。

### 推理引擎比較：Ollama vs vLLM（我們也繞了這個彎）

> 我們也天真地測試了 vLLM 作為替代推理引擎的可能性。結論：**單人使用場景下 Ollama 完勝，不用猶豫。**
...
vLLM 的核心優勢是 **continuous batching**（多人併發時效率遠超 Ollama）。但老實說，單人 Telegram bot 場景永遠只有一個請求在跑，vLLM 毫無優勢，反而因為 WSL2 的 overhead 慢到讓人崩潰。
...
## 前置準備（手動，約 5 分鐘——真的只要 5 分鐘）

在跑自動安裝 prompt 前，需先完成以下三步。做完之後 Claude Code 就能無人值守幫你搞定剩下的事。
...
> 以上三步搞定後，後續所有安裝都可以交給 Claude Code。去泡杯咖啡吧。

---

## 快速安裝：讓 Claude Code 當你的安裝工人

### Prompt A：設定 PC（Inference 機）

> 在 Mac 開一個新的 Claude Code session，把以下 prompt 貼進去。Claude Code 會透過 SSH 遠端設定 PC。完成後確認驗證報告全部通過，再執行 Prompt B。對，就是這麼簡單（理論上）。
...
> 確認 Prompt A 驗證報告全部通過後，在 Mac 開新的 Claude Code session，把以下 prompt 貼進去。如果 Prompt A 有任何一項失敗，先回去修好再來這裡。
...
> **劃重點：** 這個檔案是 tool calling 穩定的關鍵。不寫或寫得隨便，8B 模型就會開始亂搞，把 shell 指令名當 tool name、對著空氣說話之類的。如果你有自訂的 exec 指令（如智慧家居控制、SSH 到 PC），也務必寫在這裡。
...
> - `"input": ["text", "image"]` 是啟用 vision 的必要欄位，沒有它模型就看不到圖片
> - `contextWindow: 32768`（8B 模型在 16GB VRAM 下的安全值）
> - `tools.deny: []`（全開，讓模型自己決定用哪個 tool）

### 6. AGENTS.md

見 Prompt B 步驟 6。**這是 tool calling 穩定的關鍵，沒有這個檔案你的 8B 模型就會開始胡鬧。務必建立。**
...
## 快速驗證清單（架設完成後的 Checklist）

架設完成後依序確認。如果卡在某一步，翻翻下面的踩坑紀錄，大概率有答案：
...
## 踩過的坑（精選集，完整版見 [pitfalls.md](pitfalls.md)）

### #1：`~/.openclaw/` 權限問題（EACCES）
- **原因：** 用 sudo 編輯設定檔，owner 變成 root，OpenClaw 就讀不到了
...
### #3：`/new` 只顯示 heartbeat，main session 消失
- **原因：** `~/.openclaw/workspace/BOOTSTRAP.md` 存在。刪掉就好了，就是這麼無聊的答案。
- **解法：** `rm ~/.openclaw/workspace/BOOTSTRAP.md`

### #4：macOS `/usr/local/bin` 不存在（Apple Silicon）
- **解法：** wrapper script 放 `/opt/homebrew/bin/`。Apple Silicon 的世界就是不一樣。
...
- **解法：** `openclaw.json` 的 model 定義加此欄位。少一個設定差很多。

### #7：Tool calling 失效（model 輸出純文字 JSON）
- **原因 A：** 模型沒有 `tools` capability（gemma3、phi4 之類的）
...
- **規則：** 8B 模型 16 GB VRAM 建議 ctx <= 49152；14B 模型建議 ctx <= 16384。超過這個，Ollama 就會偷偷把東西丟到 CPU 上。

### #9：Thinking model 速度極慢
- **原因：** 每次生成大量內部推理 token（最多 17000+），帳面 tok/s 看起來沒差，但實際等待時間長到你以為它掛了
- **解法：** 用 instruct 變體（如 `qwen3-vl:8b-instruct`），速度快 2-8 倍

### #10：`api` 與 baseUrl 格式
- Ollama native API：`"api": "ollama"`，`baseUrl` **不加 `/v1`**。就這麼簡單，但我們卡了一小時。
...
- **現象：** gateway 啟動但什麼功能都沒有，log 默默顯示 "Config invalid"
- **解法：** 字串內 `"` 改 `\"`，用 `python3 -m json.tool` 驗證

### #13：SearXNG 不是 OpenClaw 原生 web_search provider
- 整合方式：透過 exec tool 呼叫 wrapper script，**不是** `web_search.provider`。一個 workaround，但能用就好。

### #14：換模型前必須確認 tools capability
- `ollama show MODEL | grep capabilities`，必須包含 `tools`
- 不支援的模型：gemma3、phi4、llama3.2-vision。別問我怎麼知道的。
...
- **影響：** 3B-9B 模型在此壓力下無法正確決策呼叫哪個 tool，腦容量不夠用了
- **解法：** 用 24B+ 模型（如 mistral-small3.1），或花時間好好寫 AGENTS.md 讓 8B 模型（qwen3-vl）也能穩定運作
...
想知道你的模型到底跑多快？這個 skill 自動測試本地 Ollama 模型的效能（tok/s、TTFT、VRAM 用量），還會幫你產生漂亮的 Markdown 報告。
...
OpenClaw 的本地搜尋 skill，透過 exec tool 呼叫 SearXNG 實例。免費、不需 API key、搜尋結果品質還不錯。
